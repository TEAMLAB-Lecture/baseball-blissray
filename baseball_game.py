...
def is_duplicated_number(three_digit):
    # '''
    # Input:
    #   - three_digit : 문자열로 된 의세자리 양 정수 값
...
def get_not_duplicated_three_digit_number():
    ...
    while True:
        ...
        if random_number[0]==random_number[1] or random_number[1]==random_number[2] or random_number[0]==random_number[2] :
            continue
        else :
    ...
    while True :
        ...
        if is_validated_number(user_input) == True:
            if get_strikes_or_ball(user_input,random_number)==[3,0]:
                ST = get_strikes_or_ball(user_input,random_number)[0]
                Ba = get_strikes_or_ball(user_input,random_number)[1]
                print("Strikes : {} , Balls : {}".format(ST,Ba))
                while True :
                    one_more_input=input("You win, one more(Y/N)?")
                    if is_yes(one_more_input)==True:
                        random_number = str(get_not_duplicated_three_digit_number())
